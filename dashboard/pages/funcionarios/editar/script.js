...
async function init(){
    let id_func = new URL(window.location.href).searchParams.get("funcionario");
    let func = await funcionario(id_func);

    preencher_campos(func);
    adicionar_eventos();
}

const preencher_campos = async(func) => {
    document.getElementById("primeiro_nome").value = func.primeiro_nome;
    document.getElementById("ultimo_nome").value = func.ultimo_nome;
    document.getElementById("username").value = func.username;
    ...
    document.getElementById("km_por_litro").value = func.kilometragem_por_litro;
    document.getElementById("placa").value = func.placa;
    document.getElementById("submit").value = func.id;
    preencher_select_fabricantes();
}

const preencher_select_fabricantes = async() => {
...
const salvar = async () => {
    ...
    let url = `${base_url}/api/funcionario/${document.getElementById("submit").value}`;
    let settings = {
        method: 'PUT',
        headers: {
...
const funcionario = async (id_func) => {
    let url = `${base_url}/api/funcionario/${id_func}`;
    let response = await fetch(url);
    let json = await response.json();
    return json;
}